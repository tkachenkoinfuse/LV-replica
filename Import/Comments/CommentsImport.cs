...
{
    public class CommentsImport
    {
        [DisplayName("JobID: {0}")]
        public void RequestData(string jobName, int intervalDays)
        {
            Log.Logger.Information("Comments Start!");
            ...
            do
            {
                ...
                BackgroundJob.Enqueue<CommentsImport>(j => j.RequestPart(dateStart, dateEnd));
                i++;

            }
            while (dateStart > deadline);

            Log.Logger.Information("Comments Done!");
        }
        ...
        public void RequestPart(DateTime dateStart, DateTime dateEnd)
        {
            ...
            do
            {
                ...
                List<Comments> sourceData = this.getData(dateStart, dateEnd, start);
                BackgroundJob.Enqueue<CommentsImport>(j => j.writeData(sourceData));
                start += 10000;
        ...
        public void writeData(List<Comments> sourceData)
        {
            using (var _baseContextTarget = new BaseContextTarget())
            {
                ...
                {
                    var existing = _baseContextTarget.comments.Find(table.id);
                    if (existing != null)
                    {
                        _baseContextTarget.comments.Remove(existing);
                    }
                    commentsList.Add(table);
                }
                _baseContextTarget.SaveChanges();
                _baseContextTarget.comments.AddRange(commentsList);
                _baseContextTarget.SaveChanges();
            }
        }
        private List<Comments> getData(DateTime dateStart, DateTime dateEnd, int start)
        {
            //Log.Logger.Information("start reading");
            List<Comments> comments = new();
            using (var _baseContextReplica = new BaseContextReplica())
            {
                var tableData = _baseContextReplica.comments
